...
function afficherQuantites() {
    ...
    const compteurs = panier.reduce((acc, id) => {
        acc[id] = (acc[id] || 0) + 1;
        return acc;
    }, {});

    for (const [id, quantite] of Object.entries(compteurs)) {
        console.log(`Article ${id} : ${quantite} exemplaire${quantite > 1 ? 's' : ''}`);
    }
}
...
console.log("\n--- Après ajout de l'article 105 ---");
ajouterAuPanier(105);
afficherQuantites();

console.log("\n--- Après suppression de l'article 101 ---");
retirerDuPanier(101);
afficherQuantites();
